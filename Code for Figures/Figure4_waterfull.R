...
plotMutWaterFull <- function( MutMatrix = MutMatrix , col_class = col_class , Variant_Type_Combine = Variant_Type_Combine , images_name = images_name ){
	...
	alter_fun = list(
	    ...
	    Missense_Mutation = function(x, y, w, h) {
	        grid.rect(x, y, w-unit(len, "mm"), h-unit(len, "mm"), 
	...
	col_im = colorRamp2(c(0 , 1) , c("white", col_class["IM"]) )
	col_igc = colorRamp2(c(0 , 1) , c("white", col_class["IGC"]) )
	col_dgc = colorRamp2(c(0 , 1) , c("white", col_class["DGC"]) )
	...
	lgd_all <- packLegend( ldg_from , ldg_mol , ldg_hp , ldg_drink , ldg_smoke , ldg_Variant , 
	...
	p <- oncoPrint(mut, name = "cases", 
	    ...
	    row_names_side = "left", row_names_gp = gpar(fontsize = 14) ,  
	...
	pdf(images_name , width = 25/1.9 , height = 16/1.9)
	draw(p)
	draw(lgd_all, x = unit(0.08, "npc"), y = unit(0.99, "npc"), just = c("left", "top"))
	dev.off()
...
images_name <- paste0(images_path , "Fig4A.pdf")
