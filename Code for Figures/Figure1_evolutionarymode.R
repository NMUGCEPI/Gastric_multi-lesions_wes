...
dat$gene_type <- ifelse( dat$gene %in% c("MUC6" , "BMP6" , "CFTR") , "IM\nfavored" , dat$gene_type )
dat$gene_type <- factor( dat$gene_type , levels = c("Maintained" , "GC\nfavored" , "IM\nfavored" , "Mixed"))
...
p1 <- ggplot(dat , aes(gene, weight= value , fill=variable))+
    ...
    theme(panel.background = element_rect(fill = NA, colour = "black", size = 1),
        ...
        axis.text.x = element_text(size = 13,color="black",face='bold' , angle = 90 , hjust = 1),
...
image_name <- paste0(out_path , "/Figure1_evolutionarymode.pdf")
pdf(image_name , width=4.65 ,height=4.3)
grid::grid.draw(gp)
dev.off()
